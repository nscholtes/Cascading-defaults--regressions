...
# Current script: Random  shocks, liquidity effects ON
...
tableoutput <- file.path("/Users", "nscholte", "Desktop","Research", "Ch.3 - Systemic risk", "Drafts", "Tables", "Random", "LRon/")
file.exists(tableoutput)

# Targeted shock datasets
## 1 shocked bank
dataset_RS_1_LRon  <- read.table("data_19-Sep-2017_RS_1_LRon.csv",header = TRUE,sep = ",")
## 5 shocked banks
dataset_RS_5_LRon  <- read.table("data_19-Sep-2017_RS_5_LRon.csv",header = TRUE,sep = ",")
## 10 shocked banks
dataset_RS_10_LRon  <- read.table("data_19-Sep-2017_RS_10_LRon.csv",header = TRUE,sep = ",")
...
dataset_RS_10_LRon$Diffusion <- dataset_RS_10_LRon$TotCapitalLoss/dataset_RS_10_LRon$Shock_size

# Reorder dataset
dataset_RS_1_LRon <- dplyr::select(dataset_RS_1_LRon, Diffusion, everything())
dataset_RS_5_LRon <- dplyr::select(dataset_RS_5_LRon, Diffusion, everything())
dataset_RS_10_LRon <- dplyr::select(dataset_RS_10_LRon, Diffusion, everything())
...
stargazer(dataset_RS_1_LRon[,cols_cascade],nobs = FALSE,
          ...
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="Summstats_casc_RS_1_LRon",
...
# 10 banks
stargazer(dataset_RS_10_LRon[,cols_cascade],nobs = FALSE,
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,
          covariate.labels=c("NumFail","CascSize","SimTime"),
          out=paste(tableoutput,"Summstats_casc_RS_10_LRon.tex",sep=""))
...
## Cascade output
correlationmatrix.cascade <- cor(dataset_RS_5_LRon[,cols_cascade])
...
stargazer(correlationmatrix.cascade, title="Correlation Matrix - Cascade properties (Targeted shock to 5 banks, liquidity effects on)",
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="corrmat_casc_RS_5_LRon",
          out=paste(tableoutput,"Corrmat_casc_RS_5_LRon.tex",sep=""))

## Shocked bank centralities
correlationmatrix.shock <- cor(dataset_RS_5_LRon[,cols_shock])
...
stargazer(correlationmatrix.shock, title="Correlation Matrix - Shocked bank centralities (Targeted shock to 5 banks, liquidity effects on)",
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="corrmat_casc_RS_5_LRon",
          out=paste(tableoutput,"Corrmat_shock_RS_5_LRon.tex",sep=""))  
...
RS_1_DV_dAssetPrice <- lm(-d_assetprice_abs
                           ~ Density + Average_Path_Length + Average_Clustering + Reciprocity + OutIn_Assortativity + InOut_Assortativity + OutOut_Assortativity + InIn_Assortativity
                           + Norm_Init_Capital + Norm_Init_IB_exp
                           + Indeg_cent_SB + Outdeg_cent_SB + Inclose_cent_SB + Outclose_cent_SB  + Btwn_cent_SB + Pagerank_cent_SB,
                           data = dataset_RS_1_LRon)
...
RS_5_DV_dAssetPrice <- lm(-d_assetprice_abs
                          ~ Density + Average_Path_Length + Average_Clustering + Reciprocity + OutIn_Assortativity + InOut_Assortativity + OutOut_Assortativity + InIn_Assortativity
                          + Norm_Init_Capital + Norm_Init_IB_exp
                          + Indeg_cent_SB + Outdeg_cent_SB + Inclose_cent_SB + Outclose_cent_SB  + Btwn_cent_SB + Pagerank_cent_SB,
                          data = dataset_RS_5_LRon)
...
### 1.1.4. DV = Change in asset price
RS_10_DV_dAssetPrice <- lm(d_assetprice_abs
                          ~ Density + Average_Path_Length + Average_Clustering + Reciprocity + OutIn_Assortativity + InOut_Assortativity + OutOut_Assortativity + InIn_Assortativity
                          + Norm_Init_Capital + Norm_Init_IB_exp
                          + Indeg_cent_SB + Outdeg_cent_SB + Inclose_cent_SB + Outclose_cent_SB  + Btwn_cent_SB + Pagerank_cent_SB,
                          data = dataset_RS_10_LRon)
...
stargazer(RS_1_DV_CapLoss,RS_5_DV_CapLoss,RS_10_DV_CapLoss,
          title = "Regression results - Random shock (liquidity effects on)",
          align = TRUE,report = "vc*",
          column.labels = c("1 bank", "5 banks","10 banks"),dep.var.labels   = "$CASCSIZE$",
          covariate.labels = CovLabels.Abs,
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_RS_LRon_Cascsize",
          out=paste(tableoutput,"RegResults_RS_LRon_Cascsize.tex",sep=""))
...
stargazer(RS_1_DV_NumFail,RS_5_DV_NumFail,RS_10_DV_NumFail,
          ...
          covariate.labels = CovLabels.Abs,
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_RS_LRon_Numfail",
          out=paste(tableoutput,"RegResults_RS_LRon_Numfail.tex",sep=""))
...
stargazer(RS_1_DV_Diffusion,RS_5_DV_Diffusion,RS_10_DV_Diffusion,
          ...
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_RS_LRon_Diffusion",
          out=paste(tableoutput,"RegResults_RS_LRon_Diffusion.tex",sep=""))

### 1.4.4. DV = Change in asset price
stargazer(RS_1_DV_dAssetPrice,RS_5_DV_dAssetPrice,RS_10_DV_dAssetPrice,
          title = "Regression results - Random shock (liquidity effects on)",
          align = TRUE, report = "vc*",
          column.labels = c("1 bank", "5 banks","10 banks"), dep.var.labels   = "$\\Delta ASSETPRICE$",
          covariate.labels = CovLabels.Ratios,
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_RS_LRon_dAssetPrice",
          out=paste(tableoutput,"RegResults_RS_LRon_dAssetPrice.tex",sep=""))
