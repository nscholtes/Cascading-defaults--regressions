...
# Current script: Targeted  shocks, liquidity effects OFF
...
tableoutput <- file.path("/Users", "nscholte", "Desktop","Research", "Ch.3 - Systemic risk", "Drafts", "Tables", "Targeted", "LRoff/")
file.exists(tableoutput)

# Targeted shock datasets
## 1 shocked bank
dataset_TS_1_LRoff  <- read.table("data_19-Sep-2017_TS_1_LRoff.csv",header = TRUE,sep = ",")
## 5 shocked banks
dataset_TS_5_LRoff  <- read.table("data_19-Sep-2017_TS_5_LRoff.csv",header = TRUE,sep = ",")
## 10 shocked banks
dataset_TS_10_LRoff  <- read.table("data_19-Sep-2017_TS_10_LRoff.csv",header = TRUE,sep = ",")
...
dataset_TS_10_LRoff$Diffusion <- dataset_TS_10_LRoff$TotCapitalLoss/dataset_TS_10_LRoff$Shock_size

# Reorder dataset
dataset_TS_1_LRoff <- dplyr::select(dataset_TS_1_LRoff, Diffusion, everything())
dataset_TS_5_LRoff <- dplyr::select(dataset_TS_5_LRoff, Diffusion, everything())
dataset_TS_10_LRoff <- dplyr::select(dataset_TS_10_LRoff, Diffusion, everything())
...
stargazer(dataset_TS_1_LRoff[,cols_cascade],nobs = FALSE,
          ...
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="Summstats_casc_TS_1_LRoff",
...
# 10 banks
stargazer(dataset_TS_10_LRoff[,cols_cascade],nobs = FALSE,
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,
          covariate.labels=c("NumFail","CascSize","SimTime"),
          out=paste(tableoutput,"Summstats_casc_TS_10_LRoff.tex",sep=""))
...
## Cascade output
correlationmatrix.cascade <- cor(dataset_TS_5_LRoff[,cols_cascade])
...
stargazer(correlationmatrix.cascade, title="Correlation Matrix - Cascade properties (Targeted shock to 5 banks, liquidity effects on)",
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="corrmat_casc_TS_5_LRoff",
          out=paste(tableoutput,"Corrmat_casc_TS_5_LRoff.tex",sep=""))

## Shocked bank centralities
correlationmatrix.shock <- cor(dataset_TS_5_LRoff[,cols_shock])
...
stargazer(correlationmatrix.shock, title="Correlation Matrix - Shocked bank centralities (Targeted shock to 5 banks, liquidity effects on)",
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="corrmat_casc_TS_5_LRoff",
          out=paste(tableoutput,"Corrmat_shock_TS_5_LRoff.tex",sep=""))  
...
stargazer(TS_1_DV_CapLoss,TS_5_DV_CapLoss,TS_10_DV_CapLoss,
          title = "Regression results - Targeted shock (liquidity effects off)",
          align = TRUE,report = "vc*",
          column.labels = c("1 bank", "5 banks","10 banks"),dep.var.labels   = "$CASCSIZE$",
          covariate.labels = CovLabels.Abs,
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_TS_LRoff_Cascsize",
          out=paste(tableoutput,"RegResults_TS_LRoff_Cascsize.tex",sep=""))
...
stargazer(TS_1_DV_NumFail,TS_5_DV_NumFail,TS_10_DV_NumFail,
          ...
          covariate.labels = CovLabels.Abs,
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_TS_LRoff_Numfail",
          out=paste(tableoutput,"RegResults_TS_LRoff_Numfail.tex",sep=""))
...
stargazer(TS_1_DV_Diffusion,TS_5_DV_Diffusion,TS_10_DV_Diffusion,
          ...
          omit.stat=c("LL","ser","f"), no.space=TRUE,font.size = "footnotesize",table.placement ="h!",label = "results_TS_LRoff_Diffusion",
          out=paste(tableoutput,"RegResults_TS_LRoff_Diffusion.tex",sep=""))
