...
dataset_TS_1 <- read.table("data_19-Sep-2017_TS_1_LRoff.csv",header = TRUE,sep = ",")

########################################################################
# Network measures 
########################################################################

cols_network <- c(13:20)   # Global network measures

# Summary statistics - network measures
stargazer(dataset_TS_1[,cols_network],nobs = FALSE,
...
stargazer(correlationmatrix.network, title="Correlation Matrix - Global network measures",
          font.size = "footnotesize",table.placement ="h!",no.space=TRUE,label="corrmat_network",float.env = "sidewaystable",
